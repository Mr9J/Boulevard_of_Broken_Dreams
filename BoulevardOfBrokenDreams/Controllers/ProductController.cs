...
namespace BoulevardOfBrokenDreams.Controllers
{
    ...
    public class ProductController : ControllerBase
    {
        ...
        public ProductController(MumuDbContext db, IHttpContextAccessor httpContextAccessor, IAmazonS3 s3Client)
        {
            _db = db;
        ...
        public async Task<IActionResult> PostAsync([FromBody] ProductDTO value)
        {
            ...
            var pd = _db.Products.Add(product);
            _db.SaveChanges();
            int newPjId = pd.Entity.ProjectId;
            ...
            if (newPjId > 0 && value.Thumbnail != null)
            {
                ...
                using (MemoryStream memStream = new MemoryStream(thumbnailBytes))
                {
                    ...
                    product.Thumbnail = $"https://cdn.mumumsit158.com/{key}"; // 設置圖片路徑
                    _db.SaveChanges(); // 再次保存更改
        ...
        // PUT api/<ProductController>/5
        [HttpPut("{id}")]
        public async Task<IActionResult> PutAsync(int id, [FromBody] ProductDTO value)
        {
            Product? p = _db.Products.FirstOrDefault(x => x.ProductId == id);
            if (p == null)
            {
